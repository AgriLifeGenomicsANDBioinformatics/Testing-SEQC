#!/bin/bash
set -e
#============== pathway based on the system ==================
# picard samtofastq.jar file path
SAMTOFASTQ="/v4scratch/lp364/SamToFastq.jar"
# paired-end data adaptor file
ADAPTER=/v4scratch/lp364/adaptorpe.fa
# polyA/T reference file 
ADAPTER2=/v4scratch/lp364/polyA.fa
# chrM fasta file
CMCCHRM=/v4scratch/lp364/bowtie-0.12.8/indexes/CMC_chrMidx
# total RNA fasta file including rhesus RRNA and mammamlian RRNA
RRNA=/v4scratch/lp364/bwa-0.6.2/Rfambwaidx
V4SCRATCH=/v4scratch/lp364
#============== EOF pathway based on the system ==================


usage="Usage: $0 QUALITYSCORETYPE INPUTPAIR1 INPUTPAIR2 LIBRARY
		\nThe script is aimed to clean paired-end reads by adaptor, quality, polyA, rRNA and chrM.\n\n
		Parmeters: \n
		1. Library is the prefix for the output of cleaned data. \n
		2. QualityScoreType could be phred33 or phred64 \n\n
		Before usage: \n
		1. The script assume that the following softwares are in the path:
		\n1) flexbar
		\n2) bwa
		\n3) bowtie
		\n4) samtools\n
		\n5) Trimmomatic
		2. Please change the variable of the scripts based on the pathway of the system:
		\n1) SAMTOFASTQ
		\n2) ADAPTER
		\n3) ADAPTER2
		\n4) CHRM
		\n5) RRNA \n\n
		
		Output:
		\n1) F1: reads that pass quality and adaptor trimming
		\n2) F2: reads that pass polyA trimming
		\n3) F3: reads that pass chrM filtering
		\n4) F4: reads that pass rRNA filtering
		\n5) lengthdist file: length distribution of the filtered reads
		\n6) sam file: reads that mapped to chrM or rRNA sequence
		\n
		"
# cmd: ./filteringpe.sh phred64 8 s_1.fastq s_2.fastq output/s_pair

if [ $# -ne 5 ]
    then
        echo -e $usage >&4
        exit 1
fi

QUALSCORE=$1; shift
THREADS=$1; shift
INPUT1=$1; shift
INPUT2=$1; shift
LIBRARY=$1; shift
V4SCRATCHDIR=$V4SCRATCH/$LIBRARY
mkdir -p $V4SCRATCHDIR

# important: quality score type for far and bowtie. 
# hard code defined variable:
FAR_FORMAT="fastq-sanger"
BOWTIE_QUAL="--phred33-quals"
# for CMM data, which is phred 64, 
# FAR_FORMAT="fastq"
# BOWTIE_QUAL="--phred64-quals"

getQual (){
   case $1 in
        "phred33") 
	  FAR_FORMAT="fastq-sanger"
	  BOWTIE_QUAL="--phred33-quals"
	  TRIMMOMATIC_FORMAT="phred33"
	;;
        "phred64")  
	  FAR_FORMAT="fastq"
	  BOWTIE_QUAL="--phred64-quals"
	  TRIMMOMATIC_FORMAT="phred64"
	;;
   esac
}

# function
fail () {
  echo "$1 step failed"
  exit 1
}

skip () {
  echo "$1 step skipped because already done."
}

section (){
  echo "[======$(date)======$1======]"
}

# quality score type:
getQual $QUALSCORE
echo "For far: $FAR_FORMAT"
echo "For bowtie: $BOWTIE_QUAL"
echo "Using $THREADS threads..."
# [ Quality and adaptor trimming ]
# Flexible Adapter Remover, version 2.0
# example /home/lishe/bin/far --adapters ~/genome/adaptor/pe/pe.fa --source CMM5_1.txt --source2 CMM5_2.txt --target CMM5_F1 --format fastq --cut-off 1 --min-overlap 10 --min-readlength 35 --phred-pre-trim 20 --max-uncalled 2 --adaptive-overlap yes
# section 1
# first filrter output file
ADAPTER_OUTPUT_FILE="$V4SCRATCH/${LIBRARY}_F1_2.fastq.gz"
if [[ ! -s $ADAPTER_OUTPUT_FILE ]]; then
flexbar --adapters $ADAPTER \
--source <( gunzip -c $INPUT1 ) \
--source2 <( gunzip -c $INPUT2 ) \
--target $V4SCRATCH/${LIBRARY}_F1 \
--format $FAR_FORMAT \
--threads $THREADS \
--adapter-threshold 1 \
--adapter-min-overlap 10 \
--min-readlength 35 \
--max-uncalled 5 \ || { rm -f $V4SCRATCH/${LIBRARY}_F1_{1,2}.fastq ;fail "Adaptor removing"; }
RAN_ADAPTER=1
gzip $V4SCRATCH/${LIBRARY}_F1_1*.fastq
gzip $V4SCRATCH/${LIBRARY}_F1_2*.fastq

else
	skip "Adaptor removing"
fi

# [Filter Quality Reads]
#second filter output file
TRIM_OUTPUT_FILE="$V4SCRATCH/${LIBRARY}_F2_1.fastq.gz"
if [[ $RAN_TRIM || ! -s $TRIM_OUTPUT_FILE ]]; then
java -classpath trimmomatic-0.22.jar org.usadellab.trimmomatic.TrimmomaticPE -threads $THREADS -$TRIMMOMATIC_FORMAT $V4SCRATCH/${LIBRARY}_F1_1.fastq.gz $V4SCRATCH/${LIBRARY}_F1_2.fastq.gz $V4SCRATCH/${LIBRARY}_F2_1.fastq.gz $V4SCRATCH/${LIBRARY}_F2_1_single.fastq.gz $V4SCRATCH/${LIBRARY}_F2_2.fastq.gz $V4SCRATCH/${LIBRARY}_F2_2_single.fastq.gz LEADING:20 TRAILING:20 MINLEN:35 || { rm -f $V4SCRATCH/${LIBRARY}_F2_{1,2}.fastq ; fail "trimming";}
RAN_TRIM=1
rm $V4SCRATCH/${LIBRARY}_F1_1.fastq.gz
rm $V4SCRATCH/${LIBRARY}_F1_2.fastq.gz
rm $V4SCRATCH/${LIBRARY}_F1_1.fastq.lengthdist
rm $V4SCRATCH/${LIBRARY}_F1_2.fastq.lengthdist
else
        skip "trimming..."
fi

# [ PolyA/T trimming ]
# polyA/T information
# second filter output file
POLYA_OUTPUT_FILE="$V4SCRATCH/${LIBRARY}_F3_1.fastq.gz"
# output prefix
# example: far --adapters $ADAPTER2 --source CMM5_F1_1.fastq --source2 CMM5_F1_2.fastq --target CMM5_F2 --format fastq --cut-off 0 --min-overlap 6 --min-readlength
#35 --max-uncalled 5 --adaptive-overlap no
if [[ $RAN_POLYA || ! -s $POLYA_OUTPUT_FILE ]]; then
flexbar --adapters $ADAPTER2 \
--source <( gunzip -c $V4SCRATCH/${LIBRARY}_F2_1.fastq.gz ) \
--source2 <( gunzip -c $V4SCRATCH/${LIBRARY}_F2_2.fastq.gz ) \
--target $V4SCRATCH/${LIBRARY}_F3 \
--format $FAR_FORMAT \
--threads $THREADS \
--adapter-threshold 0 \
--adapter-min-overlap 6 \
--min-readlength 35 \
--max-uncalled 5 \ || { rm -f $V4SCRATCH/${LIBRARY}_F3_{1,2}.fastq ; fail "PolyA removeing"; }
RAN_POLYA=1
gzip $V4SCRATCH/${LIBRARY}_F3_1*.fastq
gzip $V4SCRATCH/${LIBRARY}_F3_2*.fastq
rm $V4SCRATCH/${LIBRARY}_F2_1.fastq.gz
rm $V4SCRATCH/${LIBRARY}_F2_2.fastq.gz
else
	skip "PolyA removeing"
fi
# [ ChrM filtering ]
# bowtie version: bowtie version 0.12.3

bowtie ${BOWTIE_QUAL} --threads $THREADS $CMCCHRM -1 <( gunzip -c $V4SCRATCH/${LIBRARY}_F3_1.fastq.gz ) -2 <( gunzip -c $V4SCRATCH/${LIBRARY}_F3_2.fastq.gz ) --un $V4SCRATCH/${LIBRARY}_F4.fastq 
gzip $V4SCRATCH/${LIBRARY}_F4_1.fastq
gzip $V4SCRATCH/${LIBRARY}_F4_2.fastq
rm $V4SCRATCH/${LIBRARY}_F3_1.fastq.gz
rm $V4SCRATCH/${LIBRARY}_F3_2.fastq.gz
rm $V4SCRATCH/${LIBRARY}_F3_1.fastq.lengthdist
rm $V4SCRATCH/${LIBRARY}_F3_2.fastq.lengthdist
# [ rRNA filtering ]
# bwa Version: 0.5.9-r16

# bwa parameters
OPTIONS="-n 3 -o 2 -e 1 -k 2 -t $THREADS"
bwa aln $OPTIONS $RRNA $V4SCRATCH/${LIBRARY}_F4_1.fastq.gz  > $V4SCRATCH/${LIBRARY}_F5_1.sai
bwa aln $OPTIONS $RRNA $V4SCRATCH/${LIBRARY}_F4_2.fastq.gz  > $V4SCRATCH/${LIBRARY}_F5_2.sai
bwa sampe -P $RRNA $V4SCRATCH/${LIBRARY}_F5_1.sai $V4SCRATCH/${LIBRARY}_F5_2.sai $V4SCRATCH/${LIBRARY}_F4_1.fastq.gz $V4SCRATCH/${LIBRARY}_F4_2.fastq.gz  > $V4SCRATCH/${LIBRARY}_F5.sam
rm $V4SCRATCH/${LIBRARY}_F4_1.fastq.gz
rm $V4SCRATCH/${LIBRARY}_F4_2.fastq.gz
rm $V4SCRATCH/${LIBRARY}_F5_1.sai
rm $V4SCRATCH/${LIBRARY}_F5_2.sai
# after bwa alignment, parse out unmapped reads and convert back to fastq file
/v4scratch/lp364/samtools-0.1.18/samtools view -S -f 0x04 $V4SCRATCH/${LIBRARY}_F5.sam | java -Xmx2g -jar $SAMTOFASTQ INPUT=/dev/stdin VALIDATION_STRINGENCY=SILENT QUIET=true FASTQ=$V4SCRATCH/${LIBRARY}_F5_1.fastq SECOND_END_FASTQ=$V4SCRATCH/${LIBRARY}_F5_2.fastq
# [ pairing sequence ]
# pair the paired end reads after bwa using Far
# example: PairedreadFinder -s1 CMM5_F4_1.fastq -s2 CMM5_F4_2.fastq -f fastq -t1 CMM5_1.paired -t2 CMM5_2.paired -is 2
# PairedreadFinder -s1 ${LIBRARY}_F4_1.fastq -s2 ${LIBRARY}_F4_2.fastq -f fastq -t1 ${LIBRARY}_1.paired -t2 ${LIBRARY}_2.paired -is 2
gzip $V4SCRATCH/${LIBRARY}_F5_1.fastq
gzip $V4SCRATCH/${LIBRARY}_F5_2.fastq
rm $V4SCRATCH/${LIBRARY}_F5.sam
# [ program end ]
